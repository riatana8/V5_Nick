...
% 
fprintf('************************************************************\n');
fprintf('******** INFO FOR UPDATE-TARGET-PT-POSITIONS-FILE: *********\n');
fprintf('************************************************************\n\n');

fprintf('FIRST and LAST pts on Peristaltic Region (OUTER,in c++ indexing): %d and %d\n',0,NTvec(3)-1);
fprintf('FIRST and LAST pts on Peristaltic Region (INNER,in c++ indexing): %d and %d\n',NTvec(3),2*NTvec(3)-1);
...
fprintf('Outer Bottom (p0): 1st Pt = %d, 2nd Pt (p1) = %d\n',Info_Target(6),Info_Target(7));
...
function [X,Y,NVec,Info_Target] = Make_Boundary(ds,L,R_o,R_i,d,centery)
   ...
   % figure
    ...
    % figure
    plot(xS, yP_o_2,'b--'); hold on
    plot (xS, yP_i_2,'b-.'); hold on
...
for i=Nw_2:N_end_P
   yS_B2(i) = A_tilde*(xS(i)-x0)^2*(x1-xS(i))^2*exp( -(xS(i)-xC)^2 / (w/2)^2 ); 
    ...
    Nendend = Nbef+Nperi-1;
    
    %Store Info for update_target_points.C file
    Info_Target(1) = w;      %Stores width
    Info_Target(2) = R_o+0.1;    %Stores Outer Radius (+0.1 for Lindsay's code)
    Info_Target(3) = R_i+0.1;    %Stores Inner Radius (+0.1 for Lindsay's code)
